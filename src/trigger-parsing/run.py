...
def getEmailAttachment(data):
	print('receiving email from {} ({})'.format(data['from'][0], data['sender'][0]))
	print('subject: {}'.format(data['subject'][0]))
	print('email body: {}'.format(repr(data['body-plain'][0])))
	if 'attachments' not in data:
		return
	for attachment in json.loads(data['attachments'][0]):
